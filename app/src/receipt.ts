...
export const RECEIPT_PROMPT = [
  ...
  "Some receipts add a recycling fee (recyklační poplatek, often labelled 'RP') on top of the",
  "product price — a mandatory charge shown as a separate row or an extra column next to the item,",
  "with its own gross amount and VAT. Include each such fee as its own line item named",
  "'Recyklační poplatek' with its gross price and VAT rate, so the line items add up to the printed",
  "total; never drop it. Do not add it separately when it is already part of an item's printed price.",
